...
# Reconstructing the previously created YANG statement entries and compliance rules.
# Note: These entries mirror the JSON rule structures we discussed in the conversation.
# For brevity and correctness, we recreate them faithfully based on the chat-produced content.
...
# Compliance rules from earlier (RFC7952 + RFC8407 samples)
compliance_rules = [
  {
...
# Merge & deduplicate statements by name (case-insensitive)
merged_statements = {}
for s in yang_statements:
    key = s["name"].strip().lower()
    if key in merged_statements:
        # If duplicate, attempt to merge substatements and constraints conservatively
        existing = merged_statements[key]
        # merge lists uniquely
        for k in ("substatements", "constraints", "applies_to", "examples"):
            if k in s and s[k]:
                existing.setdefault(k, [])
                for item in s[k]:
                    if item not in existing[k]:
                        existing[k].append(item)
        # prefer to keep original intent, syntax, yang_version
    else:
        merged_statements[key] = s

merged_list = list(merged_statements.values())

final = {
    "metadata": {
        "source_files": ["rfc7950.txt", "rfc7952.txt", "rfc8407.txt"],
        "created_on": datetime.utcnow().isoformat() + "Z",
        "generator": "Assistant merge of RFC7950 grammar + RFC7952 & RFC8407 compliance rules"
    },
    "yang_statements": merged_list,
    "compliance_rules": compliance_rules
}

# Save merged JSON
out_path = Path("yang_rulebook_merged.json")
...
summary = {
    "file": str(out_path),
    "statements_count": len(merged_list),
    "compliance_rules_count": len(compliance_rules)
}

summary, out_path.exists()
